...
class Team(object):
    ...
    def __init__(self, teamID, UDP=False, dilationFactor=1, networkSize=4):
        ...
        for i in range(self.networkSize):
            if i != self.teamID:
                savingFilePath = os.path.join("./mtp_network_mode/savingFiles" + str(self.teamID), "team" + str(i), "received_file.txt")
                self.savingFiles[i] = FileClass(reader=False, path=savingFilePath)
                senderFolder = os.path.join("./mtp_network_mode/sendFiles" + str(self.teamID), "team" + str(i))
